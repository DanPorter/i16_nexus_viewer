...
class ScanPlotsMatplotlib(Scan):
    """
    Collection of plotting functions for NexusScan using matplotlib
    """

    def plot_dataset(self, axes=None, xaxis='axes', yaxis='signal', *args, **kwargs):
        """
        ...
        yname, ename, ydata, yerror = self.auto_data(yaxis)
        if not self._default_address['plot_errors']:
            yerror = None

        if 'label' not in kwargs.keys():
            kwargs['label'] = yname
        lines = plot_line(axes, xdata, ydata, yerror, *args, **kwargs)
        return lines
    ...
    def plot(self, xaxis='axes', yaxis='signal', *args, **kwargs):
        ...
        for yaxis in ylist:
            ...
            if not self._default_address['plot_errors']:
                yerror = None
            plot_line(ax, xdata, ydata, yerror, *args, label=yname, **kwargs)

        # Add labels
        ttl = self.title()
        ylabel = ', '.join(set(ynames))
        labels(ttl, xname, ylabel, legend=True)
        return ax

    def plot_detector(self, index=None, image_address=None,
                      xaxis='axes', axes=None, clim=None, cmap=None, colorbar=False, **kwargs):
        """
        Plot detector image in matplotlib figure
        :param index: int, detector image index, 0-length of scan, if None, use centre index
        :param image_address: location of image data in nexus file, if None, finds automatically
        ...
        labels(ttl, colorbar=colorbar, colorbar_label=image_address, axes=axes)
...
class MultiScanPlotsMatplotlib(MultiScan):
    """
    Collection of plotting functions for NexusMultiScan using matplotlib
    """
    def plot_scandata(self):
        pass
    ...
    def plot_dependence2d(self):
        pass

    def plot_dependence3d(self):
        pass
